...
JSON-CC internally uses std::intmax_t to store integral types and hence won't
compile when receiving a std::uintmax_t integer.

In other words, all integral types are converted to std::intmax_t implicitly.
